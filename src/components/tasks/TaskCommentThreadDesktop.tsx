...
export default function TaskCommentThreadDesktop({
  comments,
  commentDraft,
  isImageProcessing,
  ...
  onPickImage,
  onSubmitComment,
}: TaskCommentThreadDesktopProps) {
  return (
    <section className="task-comment-thread task-comment-thread--desktop" aria-label="Комментарии">
      <div className="task-comment-thread__list">
        {comments.map((item) => (
          <article key={item.id} className="task-comment-thread__item">
            ...
            {item.imageUrl ? (
              <img
                className="task-comment-thread__image-upload"
                src={item.imageUrl}
